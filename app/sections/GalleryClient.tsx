...
import { Flame, Flower, Gift, PenLine, Table2 } from "lucide-react";
...
export default function GalleryClient({ galleryItems }: { galleryItems: GalleryItem[] }) {
  ...
  const mobileItems = galleryItems.filter(
    (item) => item.category === mobileCategory,
  );
  const mobileItem = mobileItems[mobileImageIndex];
  const categoryIcons = [Table2, Gift, Flame, Flower, PenLine];
  ...
  return (
    <section id="galeria" className="section gallery">
      ...
      <div className="container">
        ...
        <div className="mobile-gallery" aria-live="polite">
          ...
          {mobileItem ? (
            <>
              ...
              <div className="mobile-gallery-meta">
                <span>
                  {mobileImageIndex + 1} / {mobileItems.length}
                </span>
                <span className="mobile-gallery-category-name">{mobileCategory}</span>
              </div>
            </>
          ) : (
            <p className="mobile-gallery-empty">{content.emptyState}</p>
          )}
        </div>
      </div>
    </section>
